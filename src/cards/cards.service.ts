...
export class CardsService {
  ...
  async create(user: User, createCardDto: CreateCardDto) {
     ...
     const titleExists = await this.cardsRepository.findOneWhitTitle(user.id, createCardDto.title);
     if (titleExists) throw new ConflictException("You Alredy Have a Card With This Title");
    
     const expDate = this.convertMMYYToDate(createCardDto.expirationDate)
     const isoStringExpDate = expDate.toISOString()
     return await this.cardsRepository.create(user, createCardDto, isoStringExpDate)
  }

  async findAll(user: User) {
    const cardsList = await this.cardsRepository.findAll(user.id);
    if (cardsList.length < 1) throw new NotFoundException("You Have No cards Registed.")
    const decryptedcards = cardsList.map(card => { return {...card, password: this.cryptrService.decrypt(card.password), cvv: this.cryptrService.decrypt(card.cvv)}} )
  ...
  async findOne(id: number, user: User) {
    ...
    if (card.userId !== user.id) throw new ForbiddenException({message: "You Are Not Allowed To Access This Card."})
    return {... card, password: this.cryptrService.decrypt(card.password), cvv: this.cryptrService.decrypt(card.cvv)}
  }
  ...
  const yyyyMmDdString = `20${year}-${String(parsedMonth).padStart(2, '0')}-01`;
