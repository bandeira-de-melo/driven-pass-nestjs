...
export class NotesService {
  ...
  async create(user: User, createNoteDto: CreateNoteDto) {
    const titleExists = await this.notesRepository.findOneWhitTitle(
      user.id,
      createNoteDto.title,
    );
    if (titleExists)
      throw new ConflictException('You Alredy Have a note With This Title');
  ...
  async findAll(user: User) {
    ...
    if (notesList.length < 1)
      throw new NotFoundException('You Have No Notes Registed.');
    return notesList;
  }
