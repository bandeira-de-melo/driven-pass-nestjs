...
export class CredentialsService {
  ...
  async create(user: User, createCredentialDto: CreateCredentialDto) {
    const titleExists = await this.credentialsRepository.findOneWhitTitle(
      user.id,
      createCredentialDto.title,
    );
    if (titleExists)
      throw new ConflictException(
        'You Alredy Have a Credential With This Title',
      );
  ...
  async findAll(user: User) {
    ...
    if (credentialsList.length < 1)
      throw new NotFoundException('You Have No Credentials Registed.');
    const decryptedCredentials = credentialsList.map((credential) => {
      return {
        ...credential,
  ...
  async findOne(id: number, user: User) {
    ...
    if (!credential)
      throw new NotFoundException({ message: 'Credential Not Found.' });
